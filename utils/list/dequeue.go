...
// Dequeue represents a single instance of the deque data structure. A Dequeue
...
// NewDequeue creates a new Dequeue, optionally setting the current and minimum capacity
// when non-zero values are given for these. The Dequeue instance returns
// operates on items of the type specified by the type argument. For example,
// to create a Dequeue that contains strings,
//
//	stringDeque := deque.NewDequeue[string]()
//
// To create a Dequeue with capacity to store 2048 ints without resizing, and
// that will not resize below space for 32 items when removing items:
//
//	d := deque.NewDequeue[int](2048, 32)
//
// To create a Dequeue that has not yet allocated memory, but after it does will
// never resize to have space for less than 64 items:
//
//	d := deque.NewDequeue[int](0, 64)
//
// Any size values supplied here are rounded up to the nearest power of 2.
func NewDequeue[T any](size ...int) *Dequeue[T] {
...
// Cap returns the current capacity of the Dequeue. If q is nil, q.Cap() is zero.
...
// with PopBack().
func (q *Dequeue[T]) PushBack(elem T) {
	q.growIfFull()
...
func (q *Dequeue[T]) PushFront(elem T) {
	q.growIfFull()
...
// call panics.
//
// The purpose of At is to allow Dequeue to serve as a more general purpose
...
func (q *Dequeue[T]) Set(i int, elem T) {
	if i < 0 || i >= q.count {
...
// back-to-front. Having Dequeue provide Rotate() avoids resizing that could
...
// Index returns the index into the Dequeue of the first item satisfying f(item),
// or -1 if none do. If q is nil, then -1 is always returned. Search is linear
// starting with index 0.
func (q *Dequeue[T]) Index(f func(T) bool) int {
	if q.Len() > 0 {
		modBits := len(q.buf) - 1
		for i := 0; i < q.count; i++ {
...
// returned by Front().
func (q *Dequeue[T]) RIndex(f func(T) bool) int {
	if q.Len() > 0 {
		modBits := len(q.buf) - 1
...
//
// Important: Dequeue is optimized for O(1) operations at the ends of the queue,
...
func (q *Dequeue[T]) Insert(at int, item T) {
	if at < 0 || at > q.count {
...
// constant plus linear in the lesser of the distances between the index and
// either of the ends of the queue.
func (q *Dequeue[T]) Remove(at int) T {
	if at < 0 || at >= q.Len() {
...
func (q *Dequeue[T]) SetMinCapacity(minCapacityExp uint) {
...
func (q *Dequeue[T]) prev(i int) int {
	return (i - 1) & (len(q.buf) - 1) // bitwise modulus
}

// next returns the next buffer position wrapping around buffer.
func (q *Dequeue[T]) next(i int) int {
	return (i + 1) & (len(q.buf) - 1) // bitwise modulus
}

// growIfFull resizes up if the buffer is full.
func (q *Dequeue[T]) growIfFull() {
...
func (q *Dequeue[T]) shrinkIfExcess() {
